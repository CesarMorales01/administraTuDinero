...
const Income = (params) => {
    ...
    const [total, setTotal] = useState(params.ingresos);
    const [lista, setLista] = useState(params.listaIngresos);
    ...
    useEffect(() => {
        ...
        glob.setCookie("ultimaPantalla", "Ingresos", 3600 * 60 * 24);
    }, []);
    ...
    function cargarDatos() {
        ...
        fetch(url)
            ...
            .then((json) => {
                setCargar(false);
                setTotal(json.ingresos);
                setLista(json.listaIngresos);
    ...
    function fetchEliminarIngreso(item) {
        ...
        axios
            ...
            .then((response) => {
                cargarFechas();
                setTotal(response.data.ingresos);
                setLista(response.data.listaIngresos);
                document.getElementById("btnCloseModalLoading").click();
    ...
    return (
        <AuthenticatedLayout user={params.auth} globalVars={params.globalVars}>
            ...
            ></button>
            <BotonGastoFlotante colorFondo="#79ad80" onClick={goNuevoIngreso} />
            <NuevoIngreso
